...
namespace Kryz.Utils
{
	public static class BinarySearch
	{
		...
		public static int BinarySearchLeftmost<T, TList, TComp>(this TList list, T value) where TList : IReadOnlyList<T> where TComp : IComparer<T>
		{
			return list.BinarySearchLeftmost(value, 0, list.Count, Comparer<T>.Default);
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public static int BinarySearchLeftmost<T, TList, TComp>(this TList list, T value, int index, int count) where TList : IReadOnlyList<T> where TComp : IComparer<T>
		{
